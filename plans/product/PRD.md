...
- No regular expressions anywhere in production source, including validation and parsing. Pattern matching is hard to debug: failures surface as a single non-match with no reason, and the pattern language itself becomes a second grammar to maintain. Validation is written as explicit character checks or plain conditionals so a rejection can be traced line by line (e.g. tool-name validation in `agent/tools.py`). Tests may use regexes as differential references against retired patterns only.
...
- Cross-module tunable constants live in dedicated `constants.py` modules (`ngen_weave.constants`, placeholder `ngen_weave_cli.constants`), not floating in consuming modules; user-facing settings stay in config, single-consumer privates stay local (see design/constants.md).
...
- v0.2.x execution realignment: the engine is a pure LangGraph wrapper for graph scheduling — nodes fire when triggers arrive, superstep concurrency is LangGraph's, and the engine adds no scheduling machinery on top. Relay depth-alignment for unequal fan-in was removed (static alignment is unsound under loop re-entry: stale co-parent channels silently feed joins on later iterations, and the machinery fenced basic retry loops behind compile errors). Multi-parent fan-in is compile-restricted to equal-depth acyclic joins; single-parent conditional retry loops are supported with defined semantics: re-entry fires the node on its static parent's last written output. This amends the fan-in entry above in one respect: loop iteration history is part of the defined behavior — prompt-visible content depends on declared wiring plus the retried loop's own iteration history, never on cross-node arrival races. Join freshness, dispatch delivery, and scheduling policy are deferred to `plans/design/loops-and-joins.md`.
