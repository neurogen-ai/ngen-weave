...
Option B — containerisation as the permission gate: contaierise the runtime, with either 
one container for the whole graph execution or different containers for different parts 
of the graph with different permissions/ safety guarantee requirements.
Containerising the whole application can be done by the user aswell, but that presents
friction to getting the UI to run: better if we do at least whole-graph containerisation 
on the backend and tunnel out of it to expose the API so the user / other consuming modules 
don't even notice. Marginally less granularity for permissions, though we can keep the 
existing permissions gates for softer blocks (e.g. scout tool/agent can't write). 

- seoncdary question: if we're doing this should we mount the db into the container 
or keep it external so that the code must utilise the API to edit it? We could allow for 
json crash reports written to volume mounts (or an alternative, just something fully serialised to strings to prevent code execution)
in case a process ends before the API can respond. Discuss this also. (Am i overthinking it, should we just gate the package code execution internally so it 
has to use the API to access the db anyway? How do we prevent rogue code from messing with the db if its mounted to the container?)
