...
import-time dry-run compile plus a determinism double-build check, so a broken
definition raises before any run. Model assignment never lives on classes:
bindings from the run config's models section resolve per leaf at compile
time.
...
from typing import TYPE_CHECKING, Any, ClassVar, Literal, Protocol, get_origin
...
class GraphBuilder(Protocol):
    ...
    def add_edge(
        self, src: Workflow | Literal["__start__"], dst: Workflow | Literal["__end__"]
    ) -> None: ...
...
class _Op:
    """One normalized builder call, comparable across builds for determinism."""

    name: str
    args: tuple  # class paths / END literals only; routers excluded


class _Recorder:
    """Recording sink capturing normalized ops without building anything."""
    ...
    def add_edge(self, src: Workflow | str, dst: Workflow | str) -> None:
        self.ops.append(_Op("add_edge", (_endpoint(src), _endpoint(dst))))
...
class _StateGraphAdapter:
    """Sink delegating to a real throwaway LangGraph StateGraph while recording."""
...
@dataclass
class _Wiring:
    """Topology extracted from one recorded build."""
...
def _wiring_from(ops: list[_Op], node_classes: dict[str, type[Workflow]]) -> _Wiring:
    ...
    for op in ops:
        if op.name == "add_node":
            ...
            w.edges.append((op.args[0], op.args[1]))
        else:
...
def _build_with(
    builder: Any, wf: Workflow, cls: type[Workflow]
) -> tuple[list[_Op], dict[str, type[Workflow]]]:
    try:
        wf.build(builder)
    except ConfigError:
        raise
    except Exception as exc:
        raise ConfigError(f"build() of {workflow_class_path(cls)} failed: {exc}") from exc
    return builder.ops, builder.node_classes
...
def _check_topology(w: _Wiring, cls: type[Workflow]) -> None:
    ...
    all_edges: list[tuple[str, str]] = list(w.edges)
    ...
    entries = [dst for src, dst in w.edges if src == START]
    ...
    terminals = [src for src, dst in w.edges if dst == END]
...
def validate_structure(cls: type[Workflow]) -> None:
    ...
    Instantiates cls (tunables need defaults), dry-run compiles build() twice,
    once against a recording sink and once against a throwaway StateGraph,
    compares the op sequences for determinism, then applies the structural
    and schema checks. Raises ConfigError naming the
    class path on the first problem; returns None when clean.
    """
    path = workflow_class_path(cls)
    _check_declarations(cls)
    wf = _instantiate(cls)
    recorded, rec_nodes = _build_with(_Recorder(), wf, cls)
    replayed, real_nodes = _build_with(_StateGraphAdapter(), wf, cls)
    if recorded != replayed or rec_nodes != real_nodes:
        raise ConfigError(
            f"{path}: build() is nondeterministic; it must be side-effect-free and stateless"
        )
    w = _wiring_from(recorded, rec_nodes)
    _check_leaf_composite(cls)
    _check_topology(w, cls)
